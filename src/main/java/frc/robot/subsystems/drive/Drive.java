...
public class Drive extends SubsystemBase {
  ...
  private final DifferentialDrivePoseEstimator targetSpacePoseEstimator =
      new DifferentialDrivePoseEstimator(kinematics, Rotation2d.kZero, 0.0, 0.0, Pose2d.kZero);
  ...
  public Drive(DriveIO io, GyroIO gyroIO) {
    ...
    AutoBuilder.configure(this::getPose, this::setPose,
        () -> kinematics.toChassisSpeeds(new DifferentialDriveWheelSpeeds(
            getLeftVelocityMetersPerSec(), getRightVelocityMetersPerSec())),
        (ChassisSpeeds speeds) -> runClosedLoop(speeds),
        new PPLTVController(0.02, maxSpeedMetersPerSec), ppConfig,
    ...
    sysId = new SysIdRoutine(
        ...
        new SysIdRoutine.Mechanism((voltage) -> runOpenLoop(voltage.in(Volts), voltage.in(Volts)),
            null, this));
  }
  ...
  public void periodic() {
    ...
    if (gyroInputs.connected) {
      ...
      Twist2d twist = kinematics.toTwist2d(getLeftPositionMeters() - lastLeftPositionMeters,
    ...
    targetSpacePoseEstimator.update(rawGyroRotation, getLeftPositionMeters(),
  ...
  public void setPose(Pose2d pose) {
    poseEstimator.resetPosition(rawGyroRotation, getLeftPositionMeters(), getRightPositionMeters(),
        pose);
  }

  public void setTargetSpacePose(Pose2d pose) {
    targetSpacePoseEstimator.resetPosition(rawGyroRotation, getLeftPositionMeters(),
  ...
  public void addTargetSpaceVisionMeasurement(Pose2d visionPose) {
    targetSpacePoseEstimator.addVisionMeasurement(visionPose, Timer.getFPGATimestamp());
  }
